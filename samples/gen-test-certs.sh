#!/usr/bin/env bash
# Generate test certificates for orbital-sync mTLS.
...
#   orbital-sync daemon --port 9527 --tls-ca certs/ca.pem --tls-cert certs/node-a.pem --tls-key certs/node-a-key.pem
#   orbital-sync daemon --port 9528 --tls-ca certs/ca.pem --tls-cert certs/node-b.pem --tls-key certs/node-b-key.pem
...
openssl req -x509 -newkey ec -pkeyopt ec_paramgen_curve:prime256v1 \
  ...
  -days "$DAYS" -nodes -subj "/CN=orbital-sync-ca" 2>/dev/null
...
sign_node() {
  ...
  openssl req -newkey ec -pkeyopt ec_paramgen_curve:prime256v1 \
    ...
    -nodes -subj "/CN=orbital-sync-${name}" 2>/dev/null
